...
public class BookService {
    ...
    // 대출
    // 1. 들어오는 책 이름으로 책을 찾는다
    // 2. 들어오는 유저 이름으로 유저 찾는다
    // 3-1. 혹시 지금 빌리려는 책을 다른 사람이 빌린건 아닌지 확인해서 빌렸다면 예외
    // 3-2. 빌릴 수 있는 책이면, UserLoanHistory 테이블에 데이터를 쌓아서 "대출 처리" 해주기
    ...
    public void returnBook(BookReturnRequest request) {
        ...
        // 4. is_return -> true (반납처리)
        history.doReturn();
        // 트랜잭션의 영속성으로 인해 save는 자동으로 되기때문에 굳이 save를 작성안해도 됨
        // userLoanHistoryRepository.save(history);
    }
